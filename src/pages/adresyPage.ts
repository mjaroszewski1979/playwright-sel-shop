...
export class AdresyPage {
  ...
  async isEditAddressSectionDisplayedCorrectly(): Promise<boolean> {
    try {
      const adresRozliczeniowyCorrect = await isElementVisibleWithText(
        this.adresRozliczeniowyHeader,
        'Adres rozliczeniowy'
      );
      const adresDoWysylkiCorrect = await isElementVisibleWithText(
        this.adresDoWysylkiHeader,
        'Adres do wysyłki'
      );
  ...
  async isEditingBillingAddressWorksCorrectly(): Promise<boolean> {
    try {
      ...
      const successMessageCorrect = await isElementVisibleWithText(
        this.successMessageDiv,
        'Adres został zmieniony.'
      );
      const addressStreetCorrect = await isElementVisibleWithPartialText(
        this.addressFirstSection,
        randomStreetName
      );
