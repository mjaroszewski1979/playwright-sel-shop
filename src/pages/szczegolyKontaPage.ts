...
export class SzczegolyKontaPage {
  ...
  async isAccountDetailSectionDisplayedCorrectly(): Promise<boolean> {
    try {
      ...
      const lastNameCorrect = await isElementVisibleWithCorrectValue(
        this.lastNameInput,
        'Testowy1'
      );
      const displayNameCorrect = await isElementVisibleWithCorrectValue(
        this.displayNameInput,
        'Jan Testowy1'
      );
      const userEmailCorrect = await isElementVisibleWithCorrectValue(
        this.userEmailInput,
        'UserTest1@op.pl'
      );
