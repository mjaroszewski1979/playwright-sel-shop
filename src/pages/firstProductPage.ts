...
export class FirstProductPage {
  ...
  constructor(page: Page) {
    ...
    this.goToBasketButton = page.locator('button[name="add-to-cart"]', {
      hasText: 'Dodaj do koszyka',
    });
    ...
    this.successMessage = page.locator('div.woocommerce-message', {
      hasText: 'Piłka nożna KIPSTA F100',
    });
  ...
  async isTitleMatches(): Promise<boolean> {
    try {
      await expect(this.page).toHaveTitle(
        'Piłka nożna KIPSTA F100 – Selenium Shop Automatyzacja Testów'
      );
      return true;
    } catch {
      return false;
  ...
  async isProductSectionDisplayedCorrectly(): Promise<boolean> {
    try {
      const titleCorrect = await isElementVisibleWithText(
        this.productTitleH1,
        'Piłka nożna KIPSTA F100'
      );
      const priceCorrect = await isElementVisibleWithText(this.priceAmountPara, '2,00 zł');
      const detailsCorrect = await isElementVisibleWithText(
        this.productDetailsDiv,
        'Piłka nożna Adidas Replika Liga Mistrzów rozmiar 5'
      );
      const stockCorrect = await isElementVisibleWithText(
        this.stockAvailablePara,
        'Produkt dostępny na zamówienie'
      );
      const cartCorrect = await isElementVisibleWithText(this.goToBasketButton, 'Dodaj do koszyka');
      const categoryCorrect = await isElementVisibleWithText(
        this.productCategoryDiv,
        'Kategoria: Piłki'
      );
