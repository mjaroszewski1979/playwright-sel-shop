 ...
 */
test('Verify URL of Orders page', async ({ mainPage, mojeKontoPage, zamowieniaPage }) => {
  await mainPage.goto();
 ...
 */
test('Verify structure of orders table', async ({ mainPage, mojeKontoPage, zamowieniaPage }) => {
  await mainPage.goto();
  await mainPage.gotoMojeKontoPage();
  await mojeKontoPage.login(config.username, config.password);
  await mojeKontoPage.clickZamowieniaLink();
 ...
 */
test('Verify order details', async ({ mainPage, mojeKontoPage, zamowieniaPage }) => {
  await mainPage.goto();
  await mainPage.gotoMojeKontoPage();
  await mojeKontoPage.login(config.username, config.password);
  await mojeKontoPage.clickZamowieniaLink();
